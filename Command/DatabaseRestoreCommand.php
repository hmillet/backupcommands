...
class DatabaseRestoreCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($toContinue) {
            if ($dropboxBackup) {
                ...
                $srcPath = $this->dropboxSelectFile($input, $output, $dbx_client);
     ...
     * Dropbox methods.
     */
    protected function dropboxSelectFile(InputInterface $input, OutputInterface $output, $dropbox)
    {
        ...
        do {
            ...
            if ($path !== '/') {
                $previousChoice = $path.'/..';
                array_unshift($children, $previousChoice);
            }

            $helper = $this->getHelper('question');
            $question = new ChoiceQuestion('Please select an entry :', $children, 0);
            $question->setErrorMessage('Invalid entry choice');
            $choice = $helper->ask($input, $output, $question);

            if ($previousChoice != null && $choice == $previousChoice) {
                $path = $previousPath;
            } else {
                $previousPath = $path;
                $path = rtrim($choice, '/');
